...
\begin{lstlisting}[language=perl, caption={Read mesh from a file},label={geometry-FileMeshGenerator}]
[Mesh]
    [file]
...
are to be interpreted as a sideset or as a ‘real’ element block. To introduce
this distiction Moose recognizes the \codeword{lower_dimensional_block}
codeword (although this codeword is not an official part of the MSH file
...
be pat of the physical name itself.

\begin{lstlisting}[caption={Fragment of an MSH file containing ‘lower dimensional blocks’},label={geometry-MSH-LowerDimBlock}]
$MeshFormat
4.1 0 8
$EndMeshFormat
$PhysicalNames
(*@{\raisebox{-1pt}[0pt][0pt]{$\vdots$}}@*)
0 1 "PointA"                                         (*@{$\leftarrow$ sideset (node)}@*)
1 2 "topleftedge"                                    (*@{$\leftarrow$ sideset (line)}@*)
...
2 5 "BoundaryZMin"                                   (*@{$\leftarrow$ sideset (surface)}@*)
...
TET6 elements. EDGE3 and TRI6 elements can currenty be used in a Moose-app by
...
Moose does not explicity support groups geometrical objects but allows to
...
\begin{lstlisting}[language=Perl, caption={Moose input file fragment: variable holding several block names and fake-user},label={geometry-moose-variable-groups}]
[Mesh]
