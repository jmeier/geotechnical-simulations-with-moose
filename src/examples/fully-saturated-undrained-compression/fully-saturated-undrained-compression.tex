...
$\SI{6}{\metre} \times \SI{6}{\metre} \times \SI{6}{\metre}$ fully saturated
block is to be simulated as a variant of an Oedometer test. While the
(vertical) sides are fixed horizontally and the bottom is fixed vertically, the
top surface is lowered by $\SI{0.01}{\metre}=\SI{1}{\centi\metre}$. Initially,
the block is loaded only by gravity. The soil material is assumed to be purely
linear elastic and fully saturated with water. Material parameters are given in
...
Using the values of \autoref{fully-saturated-undrained-compression:material-parameters}
for the second part of \autoref{fully-saturated-undrained-compression:stiffness-modulus}
leads to:
\begin{equation}
...
\autoref{fully-saturated-undrained-compression:pore-pressure-parameter}
can be used:
...
\autoref{fully-saturated-undrained-compression:pore-pressure-parameter}
can be rearranged to calculate the pore pressure $u$:
...
Residuals for the fluid (water) for
$ \epsilon_\mathrm{f} \equiv \qty[per-mode = symbol]{10}{\pascal\per\metre} $
may be estimated as:
\begin{equation}
    \begin{split}
...
‘fully-saturated-undrained-compression-NEWTON.i’ and ‘fully-saturated-undrained-compression-PJFNK.i’.
...
\autoref{fully-saturated-undrained-compression:moose-results}
(both solvers used lead to the same results within the specified precision).
Performance data can be found in
...
name of ‘fully-saturated-undrained-compression.p3dlog’. \fileattachment{\currfiledir/fully-saturated-undrained-compression.p3dlog}{fully-saturated-undrained-compression.p3dlog}

Selected results of this Plaxis model are summarized in \autoref{fully-saturated-undrained-compression:plaxis-results}.
Performance data can be found in \autoref{fully-saturated-undrained-compression:plaxis-performance}.
